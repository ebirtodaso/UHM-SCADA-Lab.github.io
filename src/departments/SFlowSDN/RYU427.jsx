...
const RYU427 = () => {
  ...
  return (
    <span>
      <Link to="" onClick={handleShow}>RYU - 4/27/2022</Link>
      ...
      <Offcanvas show={show} onHide={handleClose} style={{ maxWidth: '800px', width: '100%' }}>
        ...
        <Offcanvas.Body>
          <p>This is where we left off at the end of the Spring 2022 semester, with only a minor edit to tidy it up a bit. Comments only document the changes from the provided (from Ryu github) example program.</p>
          <pre className="border">{RYUProgram}</pre>
        </Offcanvas.Body>
      </Offcanvas>
    </span>
  );

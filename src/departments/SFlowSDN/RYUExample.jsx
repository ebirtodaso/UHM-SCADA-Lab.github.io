...
const RYUExample = () => {
  ...
  return (
    <span>
      <Link to="" onClick={handleShow}>RYU Example Program</Link>
      ...
      <Offcanvas show={show} onHide={handleClose} style={{ maxWidth: '800px', width: '100%' }}>
        ...
        <Offcanvas.Body>
          ...
          <pre className="border">{handshake}</pre>
          <h3>Add Flow Function</h3>
          <pre className="border">{addFlow}</pre>
          <h3>Delete Flow Function</h3>
          <pre className="border">{deleteFlow}</pre>
          <h3>Packet In Event Handler</h3>
          <pre className="border">{packetHandler}</pre>
        </Offcanvas.Body>
      </Offcanvas>
    </span>
  );
